...
if result:
    ...
    if result.get("error"):
        ...
        severity_colors = {"P0": "#dc3545", "P1": "#fd7e14", "P2": "#ffc107", "P3": "#28a745"}
        ...
        if comments_analysis:
            ...
            if details:
                with st.expander(f"查看 {len(details)} 条评论详情", expanded=False):
                    for d in details:
                        emoji_map = {"正面": "🟢", "中性": "🟡", "负面": "🔴"}
        ...
        with st.expander("点击展开纠偏表单", expanded=False):
            ...
            corr_summary = st.text_area("摘要", value=result.get("摘要", ""), key="corr_summary")
            corr_reason = st.text_area("严重度理由", value=result.get("严重度理由", ""), key="corr_reason")
            ...
            corr_note = st.text_input("纠偏备注（可选）", placeholder="为什么这样修正...", key="corr_note")

            if st.button("保存纠偏", type="primary"):
                # 构建人工修正版本
